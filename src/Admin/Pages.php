...
final class Pages {
    ...
    private function report_page( string $group, string $default ): void {
        $report_id = isset( $_GET['report'] ) && is_scalar( $_GET['report'] ) ? sanitize_key( wp_unslash( (string) $_GET['report'] ) ) : $default;
        $saved_id = isset( $_GET['saved_report'] ) && is_scalar( $_GET['saved_report'] ) ? absint( $_GET['saved_report'] ) : 0;
        $saved_row = $saved_id ? $this->saved->find( $saved_id ) : null;
        if ( $saved_row ) { $report_id = sanitize_key( (string) $saved_row['report_id'] ); }
        ...
        if ( ! $definition || ! Capabilities::current_user_can_report( $definition ) ) {
            wp_die( esc_html__( 'You do not have permission to view this report.', 'woocommerce-advanced-reports' ) );
        }
        $calendar = new Calendar();
        $stored_filters = $saved_row && $report_id === (string) $saved_row['report_id'] ? json_decode( $saved_row['filters'], true ) : null;
        $filter = is_array( $stored_filters ) ? ReportFilter::from_storage( $stored_filters, $calendar ) : ReportFilter::from_request( $_GET, $calendar );
        $data = $this->engine->run( $report_id, $filter, true, true );
        $tabs = $this->engine->registry()->by_group( $group ); $raw_filters = $filter->to_request_array( $calendar );
    ...
    public function saved(): void {
        ...
        foreach ( $rows as $row ) {
            $def = $this->engine->registry()->get( $row['report_id'] );
            $url = $def && Capabilities::current_user_can_report( $def ) ? add_query_arg( array( 'page'=>$this->page_for_group( $def['group'] ), 'report'=>$row['report_id'], 'saved_report'=>(int)$row['id'] ), admin_url( 'admin.php' ) ) : '';
            $delete = $this->delete_form( 'wcar_delete_saved_report', (int) $row['id'], 'wcar_delete_saved_' . (int) $row['id'] );
            $open = $url ? '<a class="button" href="' . esc_url( $url ) . '">' . esc_html__( 'Open', 'woocommerce-advanced-reports' ) . '</a> ' : '';
            echo '<tr><td><strong>' . esc_html( $row['name'] ) . '</strong></td><td>' . esc_html( $def['title'] ?? $row['report_id'] ) . '</td><td>' . esc_html( $row['created_at'] ) . '</td><td>' . $open . $delete . '</td></tr>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- action markup is built and escaped internally.
        }
    ...
    public function scheduled(): void {
        ...
        echo '<div class="wrap wcar-wrap"><h1>' . esc_html__( 'Scheduled Reports', 'woocommerce-advanced-reports' ) . '</h1><p>' . esc_html__( 'Rolling schedules automatically use yesterday for daily reports, the previous 7 days for weekly reports, and the previous month in the active calendar for monthly reports.', 'woocommerce-advanced-reports' ) . '</p><div class="wcar-card"><table class="widefat striped"><thead><tr><th>' . esc_html__( 'Name', 'woocommerce-advanced-reports' ) . '</th><th>' . esc_html__( 'Report', 'woocommerce-advanced-reports' ) . '</th><th>' . esc_html__( 'Cadence', 'woocommerce-advanced-reports' ) . '</th><th>' . esc_html__( 'Recipients', 'woocommerce-advanced-reports' ) . '</th><th>' . esc_html__( 'Last Run', 'woocommerce-advanced-reports' ) . '</th><th>' . esc_html__( 'Next Run', 'woocommerce-advanced-reports' ) . '</th><th>' . esc_html__( 'Last Error', 'woocommerce-advanced-reports' ) . '</th><th></th></tr></thead><tbody>';
        foreach ( $rows as $row ) { $def=$this->engine->registry()->get($row['report_id']); $delete=$this->delete_form('wcar_delete_schedule',(int)$row['id'],'wcar_delete_schedule_'.(int)$row['id']); echo '<tr><td><strong>'.esc_html($row['name']).'</strong></td><td>'.esc_html($def['title']??$row['report_id']).'</td><td>'.esc_html(ucfirst($row['cadence'])).' '.esc_html($row['run_time']).'</td><td>'.esc_html($row['recipients']).'</td><td>'.esc_html($row['last_run']?:'—').'</td><td>'.esc_html($row['next_run']?:'—').'</td><td>'.esc_html($row['last_error']?:'—').'</td><td>'.$delete.'</td></tr>'; } // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- delete form is built and escaped internally.
        if(!$rows){echo '<tr><td colspan="8">'.esc_html__('No scheduled reports yet. Create one from any report page.','woocommerce-advanced-reports').'</td></tr>';}
        echo '</tbody></table></div></div>';
    }

    public function exports(): void {
        if ( ! current_user_can( Capabilities::EXPORT ) ) { wp_die( esc_html__( 'Permission denied.', 'woocommerce-advanced-reports' ) ); }
        $rows=$this->exports->history(); echo '<div class="wrap wcar-wrap"><h1>'.esc_html__('Export History','woocommerce-advanced-reports').'</h1><div class="wcar-card"><table class="widefat striped"><thead><tr><th>'.esc_html__('Report','woocommerce-advanced-reports').'</th><th>'.esc_html__('Format','woocommerce-advanced-reports').'</th><th>'.esc_html__('Status','woocommerce-advanced-reports').'</th><th>'.esc_html__('Created','woocommerce-advanced-reports').'</th><th>'.esc_html__('Expires','woocommerce-advanced-reports').'</th><th>'.esc_html__('Error','woocommerce-advanced-reports').'</th><th></th></tr></thead><tbody>';
        foreach($rows as $row){$def=$this->engine->registry()->get($row['report_id']); $down=wp_nonce_url(admin_url('admin-post.php?action=wcar_download_export&id='.(int)$row['id']),'wcar_download_export_'.(int)$row['id']); $del=$this->delete_form('wcar_delete_export',(int)$row['id'],'wcar_delete_export_'.(int)$row['id']); $can_download='ready'===$row['status']&&$def&&Capabilities::current_user_can_report($def); echo '<tr><td>'.esc_html($def['title']??$row['report_id']).'</td><td>'.esc_html(strtoupper($row['format'])).'</td><td>'.esc_html(ucfirst($row['status'])).'</td><td>'.esc_html($row['created_at']).'</td><td>'.esc_html($row['expires_at']?:'—').'</td><td>'.esc_html($row['error_message']?:'—').'</td><td>'.($can_download?'<a class="button" href="'.esc_url($down).'">'.esc_html__('Download','woocommerce-advanced-reports').'</a> ':'').$del.'</td></tr>';} // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- delete form is built and escaped internally.
        if(!$rows){echo '<tr><td colspan="7">'.esc_html__('No exports yet.','woocommerce-advanced-reports').'</td></tr>';} echo '</tbody></table></div></div>';
    }

    public function settings(): void {
        if ( ! current_user_can( Capabilities::SETTINGS ) ) { wp_die( esc_html__( 'Permission denied.', 'woocommerce-advanced-reports' ) ); }
    ...
    private function delete_form(string $action,int $id,string $nonce_action):string{return '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'" class="wcar-inline-form"><input type="hidden" name="action" value="'.esc_attr($action).'"><input type="hidden" name="id" value="'.$id.'">'.wp_nonce_field($nonce_action,'_wcar_delete_nonce_'.$id,false,false).'<button class="button wcar-confirm-delete" type="submit">'.esc_html__('Delete','woocommerce-advanced-reports').'</button></form>';}
