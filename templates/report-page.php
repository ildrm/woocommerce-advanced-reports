...
$date_from = isset( $_GET['date_from'] ) ? sanitize_text_field( wp_unslash( $_GET['date_from'] ) ) : $calendar->format( $filter->from );
$date_to   = isset( $_GET['date_to'] ) ? sanitize_text_field( wp_unslash( $_GET['date_to'] ) ) : $calendar->format( $filter->to );
$money_keys = array( 'gross_sales','net_sales','net_collected','refunds','refund','discounts','discount','shipping','shipping_revenue','avg_shipping','tax','product_tax','shipping_tax','total_tax','aov','subtotal','total','order_total','revenue','order_value','refund_amount','regular_price','sale_price','price','retail_value','cost_value','gross_spend','net_spend','lifetime_revenue','avg_selling_price' );
$format_cell = static function ( string $key, $value, array $row ) use ( $money_keys ) {
    ...
    if ( 'repeat_rate' === $key && is_numeric( $value ) ) { return \WCAR\Support\Format::percent( (float) $value ); }
...
<div class="wrap wcar-wrap" id="wcar-report-root" data-report-id="<?php echo esc_attr($report_id); ?>">
        ...
        <div class="wcar-actions wcar-no-print">
            <?php if ( current_user_can( \WCAR\Security\Capabilities::EXPORT ) ) :
                foreach ( array( 'csv'=>'CSV','xlsx'=>'XLSX' ) as $fmt=>$label ) {
                    $url = add_query_arg( array_merge( array( 'action'=>'wcar_export','report_id'=>$report_id,'format'=>$fmt ), $raw_filters ), admin_url( 'admin-post.php' ) );
                    $url = wp_nonce_url( $url, 'wcar_export' );
                    echo '<a class="button" href="'.esc_url($url).'">'.esc_html(sprintf(__('Export %s','woocommerce-advanced-reports'),$label)).'</a> ';
                }
            endif; ?>
            <?php if ( current_user_can( \WCAR\Security\Capabilities::EXPORT ) ) : ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="wcar-inline-form"><?php wp_nonce_field('wcar_queue_export'); ?><input type="hidden" name="action" value="wcar_queue_export"><input type="hidden" name="report_id" value="<?php echo esc_attr($report_id); ?>"><input type="hidden" name="format" value="xlsx"><input type="hidden" name="filters" value="<?php echo esc_attr(wp_json_encode($raw_filters)); ?>"><button class="button" type="submit"><?php esc_html_e('Queue XLSX','woocommerce-advanced-reports'); ?></button></form><?php endif; ?>
    ...
    <form method="get" action="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>" class="wcar-card wcar-filter-form wcar-no-print">
        ...
        foreach($ranges as $label=>$range){$args=array_merge(array('page'=>$page_slug,'report'=>$report_id),$raw_filters,array('date_from'=>$calendar->format($range[0]),'date_to'=>$calendar->format($range[1])));unset($args['paged']);echo '<a class="button button-small" href="'.esc_url(add_query_arg($args,admin_url('admin.php'))).'">'.esc_html($label).'</a> ';}
        ?>
        </div>
        ...
        <details class="wcar-advanced-filters"><summary><?php esc_html_e('Advanced filters','woocommerce-advanced-reports'); ?></summary><div class="wcar-filter-grid">
            <label><span><?php esc_html_e('Product IDs','woocommerce-advanced-reports'); ?></span><input type="text" name="product_csv" value="<?php echo esc_attr(implode(',',$filter->product_ids)); ?>" data-wcar-list-target="product" placeholder="12,34,56"><input type="hidden" name="product[]" value=""></label>
            <label><span><?php esc_html_e('Category','woocommerce-advanced-reports'); ?></span><select name="category[]" multiple><?php foreach(get_terms(array('taxonomy'=>'product_cat','hide_empty'=>false)) as $term):?><option value="<?php echo esc_attr($term->term_id); ?>" <?php selected(in_array((int)$term->term_id,$filter->category_ids,true)); ?>><?php echo esc_html($term->name); ?></option><?php endforeach; ?></select></label>
    ...
    <?php if('dashboard'===$report_id && !empty($data['charts'])):?><div class="wcar-chart-grid"><div class="wcar-card"><h2><?php esc_html_e('Sales Trend','woocommerce-advanced-reports'); ?></h2><canvas class="wcar-chart" data-chart="trend"></canvas></div><div class="wcar-card"><h2><?php esc_html_e('Order Status','woocommerce-advanced-reports'); ?></h2><canvas class="wcar-chart" data-chart="status"></canvas></div><div class="wcar-card"><h2><?php esc_html_e('Top Products','woocommerce-advanced-reports'); ?></h2><canvas class="wcar-chart" data-chart="products"></canvas></div><div class="wcar-card"><h2><?php esc_html_e('Customer Mix','woocommerce-advanced-reports'); ?></h2><canvas class="wcar-chart" data-chart="customers"></canvas></div></div><script type="application/json" id="wcar-chart-data"><?php echo wp_json_encode($data['charts']); ?></script><?php endif; ?>
    ...
    <div class="wcar-tools-grid wcar-no-print">
        <div class="wcar-card"><h2><?php esc_html_e('Save this report','woocommerce-advanced-reports'); ?></h2><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><?php wp_nonce_field('wcar_save_report'); ?><input type="hidden" name="action" value="wcar_save_report"><input type="hidden" name="report_id" value="<?php echo esc_attr($report_id); ?>"><input type="hidden" name="filters" value="<?php echo esc_attr(wp_json_encode($raw_filters)); ?>"><input type="text" name="name" required placeholder="<?php esc_attr_e('e.g. Monthly Mobile Sales','woocommerce-advanced-reports'); ?>"> <button class="button" type="submit"><?php esc_html_e('Save Report','woocommerce-advanced-reports'); ?></button></form></div>
        <?php if(current_user_can(\WCAR\Security\Capabilities::EXPORT)):?><div class="wcar-card"><h2><?php esc_html_e('Schedule this report','woocommerce-advanced-reports'); ?></h2><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="wcar-schedule-form"><?php wp_nonce_field('wcar_create_schedule'); ?><input type="hidden" name="action" value="wcar_create_schedule"><input type="hidden" name="report_id" value="<?php echo esc_attr($report_id); ?>"><input type="hidden" name="filters" value="<?php echo esc_attr(wp_json_encode($raw_filters)); ?>"><input name="name" required placeholder="<?php esc_attr_e('Schedule name','woocommerce-advanced-reports'); ?>"><input name="recipients" type="text" required value="<?php echo esc_attr(wp_get_current_user()->user_email); ?>" placeholder="email@example.com"><select name="cadence"><option value="daily">Daily</option><option value="weekly" selected>Weekly</option><option value="monthly">Monthly</option></select><input type="time" name="run_time" value="08:00"><select name="format"><option value="xlsx">XLSX</option><option value="csv">CSV</option></select><select name="period_mode"><option value="rolling"><?php esc_html_e('Rolling period','woocommerce-advanced-reports'); ?></option><option value="fixed"><?php esc_html_e('Fixed current dates','woocommerce-advanced-reports'); ?></option></select><button class="button" type="submit"><?php esc_html_e('Create Schedule','woocommerce-advanced-reports'); ?></button></form></div><?php endif; ?>
    </div>
</div>
